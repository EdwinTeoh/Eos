						...
						std::visit([&](auto&& Value)
							{
								...
								else if constexpr (std::is_same_v<T, string_t>)
								{
									...
									if (E->componentType == COMPONENTNAME::Comp_Static_Mesh_Component)
									{
										...
										ImGui::LabelText("", "Select model from Asset Library");

										// Extract out list of models from asset manager and display them in the UI
										if (ImGui::BeginCombo("Selection", Value.c_str()))
										{
											int selected_idx = 0;
											std::vector<std::string> keys = AssetManager::GetInstance().GetStoredStaticMeshNames();
											
											// Find index of element if exists, else remain as default value of 0
											std::vector<std::string>::iterator it = std::find(keys.begin(), keys.end(), Value);;
											if (it != keys.end())
												selected_idx = std::distance(keys.begin(), it);

											// Populate combo box with options obtained from AssetManager
										...
										// Clear the choice from the combobox
