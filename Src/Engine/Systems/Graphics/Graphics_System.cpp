...
void Rendering_System::Init()
{
	...
	diffuseTexture = createTexturefromfile("Assets/Textures/Surge_split.png");
...
void Rendering_System::Update()
{
	...
	//Normal pipeline
	{
		for (auto mesh = meshes.begin(); mesh != meshes.end(); mesh++)
		{
			if (GET_ENTITY_MANAGER.getActive(mesh->first))
			{
				...
				mesh->second.get()->_shader_choice.Bind();
				mesh->second.get()->_shader_choice.SetMat4("projection", projection);
				mesh->second.get()->_shader_choice.SetMat4("view", view);

				glm::mat4 cubeModel = tc->GetModelMatrix();


				mesh->second.get()->_shader_choice.SetMat4("model", cubeModel);
				mesh->second.get()->_model_choice.Draw(mesh->second.get()->_shader_choice);
				mesh->second.get()->_shader_choice.unBind();
	...
	else  if (blinnPhong)//TODO Blinn
	{
		...
		for (auto mesh = meshes.begin(); mesh != meshes.end(); mesh++)
		{
			if (GET_ENTITY_MANAGER.getActive(mesh->first))
			{
				...
				mesh->second.get()->_shader_choice.SetInt("diffuseMap", 0);
				mesh->second.get()->_shader_choice.SetInt("normalMap", 1);
				...
				mesh->second.get()->_shader_choice.Bind();
				mesh->second.get()->_shader_choice.SetMat4("projection", projection);
				mesh->second.get()->_shader_choice.SetMat4("view", _view);

				glm::mat4 cubeModel = tc->GetModelMatrix();
				mesh->second.get()->_shader_choice.SetMat4("model", cubeModel);
				mesh->second.get()->_shader_choice.SetVec3("viewPos", m_persCamera.GetPosition());
				mesh->second.get()->_shader_choice.SetVec3("lightPos", lightPos);
				...
				mesh->second.get()->_shader_choice.SetMat4("model", model);
				mesh->second.get()->_model_choice.Draw(mesh->second.get()->_shader_choice);
				mesh->second.get()->_shader_choice.unBind();
	...
	for (auto stat_mesh = static_meshes.begin(); stat_mesh != static_meshes.end(); stat_mesh++)
	{
		if (GET_ENTITY_MANAGER.getActive(stat_mesh->first))
		{
			...
			// Lights
			// Protection from creating scene of static meshes without any lights
			if (lights.size() && lightPosition.size() && lightColors.size())
			{
	...
	for (auto camera = cam.begin(); camera != cam.end(); camera++)
	{
		if (GET_ENTITY_MANAGER.getActive(camera->first))
		{
			if (camera->second->GetActive() && !m_persCamera.m_active)
			{
				...
				float distancefromplayer = 15.0f;
	...
	{
		if (GET_ENTITY_MANAGER.getActive(sprite.first))
		{
			bill_shader.Bind();
			...
			if (sprite.second->GetSprite())
				glBindTexture(GL_TEXTURE_2D, sprite.second->GetSprite()->GetID());
			...
			bill_shader.SetMat4("u_Transform", tc->GetModelMatrix());
			...
			bill_shader.SetVec3("BillboardPos", sprite.second->GetBillBoardPos());
			bill_shader.SetVec2("BillboardSize", sprite.second->GetBillBoardSize());
