...
void FontCompiler::readTTF_to_MSDF(const std::string& filepath, float emSizeMin)
{
    ...
    if (ft)
    {
        ...
        if (font)
        {
            ...
            if (glyphsLoaded < 0)
            {
                PE_ERROR("Could not load glyphs from font {}", filepath.c_str());
                return;
            }

            if (glyphsLoaded < msdf_atlas::Charset::ASCII.size())
            {
                PE_WARN("Font \"{}\" is missing {} codepoints!  Codepoints:", filepath.c_str(), msdf_atlas::Charset::ASCII.size() - glyphsLoaded);
                bool first = true;
    ...
    if (glyphs.empty())
    {
        PE_ERROR("No glyphs loaded from font \"{}\"", filepath.c_str());
        return;
    }
    else
    {
    ...
    {
        PE_WARN("Unable to pack glyphs into atlas, Unpacked glyphs: {}", glyphPackFailureCount);
    }
    ...
    msdfgen::savePng(generator.atlasStorage(), "fontTFF/font-sdf-image.png");
