	...
	bool saveMesh, bool saveSkeleton, bool saveAnimation, bool saveScene)
{
	...
	if (std::filesystem::create_directories("Assets\\Animations\\"))
		PE_CORE_TRACE("Returned true");

	// open a custom file
	std::string baseFileName = "Assets\\Animations\\" + Eos::FileManager::GetFileNameWithoutExtension(path);
	...
	if (saveScene)
	{
		...
		{
			// Not open
			PE_ERROR("Scene File Saving Failed");
		}
	}
	...
	if (saveSkeleton)
	{
		...
		{
			PE_ERROR("Saving skeleton failed");
		}
	}
	...
	if (saveAnimation)
	{
		...
		{
			PE_ERROR("Animation saving failed");
		}
	}
}
