...
#include <vector>
#include "Src\Engine\Systems\Graphics\Itexture.h"
#include <iostream>
...
void Smoke_Component::compileAndLinkShader()
{
	try
	{
		prog.compileShader("shaders/smoke_vert.glsl");
		prog.compileShader("shaders/smoke_frag.glsl");
		...
		flatProg.compileShader("shaders/smoke_vert.glsl");
		flatProg.compileShader("shaders/smoke_frag.glsl");
		flatProg.link();
	}
	catch (const std::runtime_error& e) 
	{
		PE_CORE_ERROR(e.what());
		exit(EXIT_FAILURE);
	}
}
...
void Smoke_Component::initScene()
{
	...
	textureID = UTexture::loadTexture("Assets/Textures/smoke.png");
...
void Smoke_Component::SetFilePath(std::string& path)
{
	...
	if (path.compare(filePath))
	{
		...
		textureID = UTexture::loadTexture(path.c_str());
	}
}
