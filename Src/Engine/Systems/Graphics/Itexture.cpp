...
/*static*/
GLuint UTexture::loadTexture(const std::string& fName) {
    int width, height;
    unsigned char* data = UTexture::loadPixels(fName, width, height);
    uint32_t tex = 0;
    if (nullptr != data) {
        glGenTextures(1, &tex);
        ...
        UTexture::deletePixels(data);
    }

    return tex;
}

void UTexture::deletePixels(unsigned char* data) {
    stbi_image_free(data);
}

unsigned char* UTexture::loadPixels(const std::string& fName, int& width, int& height, bool flip) {
    int bytesPerPix;
...
GLuint UTexture::loadCubeMap(const std::string& baseName, const std::string& extension) {
    GLuint texID;
    ...
    GLubyte* data = UTexture::loadPixels(texName, w, h, false);
    ...
    for (int i = 1; i < 6; i++) {
        ...
        data = UTexture::loadPixels(texName, w, h, false);
...
GLuint UTexture::loadHdrCubeMap(const std::string& baseName) {
    GLuint texID;
    glGenTextures(1, &texID);
    glBindTexture(GL_TEXTURE_CUBE_MAP, texID);
